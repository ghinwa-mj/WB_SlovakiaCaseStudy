#!/usr/bin/env python3
"""
Gemini + GPT Full Dataset Analysis Script

This script runs sentiment analysis on the ENTIRE dataset using:
- Gemini 2.5 Pro
- GPT-4.1 
- GPT-5.1

Uses the exact same prompt as the original analysis.
"""
...
from apikeys import open_ai_api_key, gemini_api_key
...
class MultiModelAnalyzer:
    """Analyze project recommendations using Gemini, GPT-4.1, and GPT-5.1"""
    ...
    async def analyze_all_models(self, project_name: str, document_id: str, pdf_text: str) -> Dict:
        """Run analysis on all three models in parallel"""
...
class FullDatasetProcessor:
    """Main processor for full dataset sentiment analysis"""
    ...
    async def process_all_projects(self, df: pd.DataFrame) -> List[Dict]:
        """Process ALL projects in the dataset"""
        results = []
        ...
        for idx, row in df.iterrows():
            try:
                logger.info(f"Processing project {idx + 1}/{total_projects}")
                ...
                if (idx + 1) % 10 == 0:
                    ...
                    logger.info(f"Progress: {idx + 1}/{total_projects} projects processed, {successful} successful")
...
async def test_trial():
    ...
    logger.info("Starting test trial with Gemini, GPT-4.1, and GPT-5.1...")
...
async def main():
    ...
    output_path = f"/Users/ghinwamoujaes/Desktop/World Bank/Code/slovakia_CaseStudy/gemini_gpt_full_analysis_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    
    # Initialize processor
    processor = FullDatasetProcessor(project_summary_path, pdf_folder_path)
    ...
    try:
        ...
        logger.info(f"Processing ALL {len(df)} projects in the dataset...")
        
        # Process all projects
        logger.info("Starting analysis...")
        results = await processor.process_all_projects(df)
